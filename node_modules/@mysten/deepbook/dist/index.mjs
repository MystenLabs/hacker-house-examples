...
var NORMALIZED_SUI_COIN_TYPE = normalizeStructTag(SUI_TYPE_ARG);
...
var DUMMY_ADDRESS = normalizeSuiAddress("0x0");
...
var DeepBookClient = class {
   ...
   */
  constructor(suiClient = new SuiClient({ url: getFullnodeUrl("testnet") }), accountCap = void 0, currentAddress = DUMMY_ADDRESS, clientOrderId = 0) {
  ...
  async getAllPools(input) {
    ...
    const pools = resp.data.map((event) => {
      ...
      return {
        ...
        baseAsset: normalizeStructTag2(rawEvent.base_asset.name),
        quoteAsset: normalizeStructTag2(rawEvent.quote_asset.name)
      };
    });
    return {
      data: pools,
  ...
  async getLevel2BookStatus(poolId, lowerPrice, higherPrice, side) {
    ...
    const results = (await this.suiClient.devInspectTransactionBlock({
      ...
      ([bytes, _]) => bcs.de("vector<u64>", Uint8Array.from(bytes)).map((s) => BigInt(s))
    );
