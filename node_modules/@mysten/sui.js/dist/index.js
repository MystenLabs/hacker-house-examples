...
var import_bcs24 = require("@mysten/bcs");
...
function mnemonicToSeedHex(mnemonics) {
  return (0, import_bcs13.toHEX)(mnemonicToSeed(mnemonics));
}
...
var BaseSigner = class {
  ...
  async signWithIntent(bytes, intent) {
    const intentMessage = messageWithIntent(intent, bytes);
    const digest = (0, import_blake2b2.blake2b)(intentMessage, { dkLen: 32 });
    ...
    return {
      signature,
      bytes: (0, import_bcs15.toB64)(bytes)
    };
  }
  /**
...
function toMultiSigAddress(pks, threshold) {
  ...
  return normalizeSuiAddress((0, import_utils4.bytesToHex)((0, import_blake2b4.blake2b)(tmp.slice(0, i), { dkLen: 32 })));
}
function combinePartialSigs(sigs, pks, threshold) {
  ...
  tmp.set([SIGNATURE_SCHEME_TO_FLAG["MultiSig"]]);
  tmp.set(bytes, 1);
  return (0, import_bcs18.toB64)(tmp);
}
function decodeMultiSig(signature) {
  const parsed = (0, import_bcs18.fromB64)(signature);
...
var getMasterKeyFromSeed = (seed) => {
  ...
  const I = h.update((0, import_bcs20.fromHEX)(seed)).digest();
...
var Ed25519Keypair = class _Ed25519Keypair extends Keypair {
  ...
  export() {
    return {
      schema: "ED25519",
      privateKey: (0, import_bcs21.toB64)(this.keypair.secretKey.slice(0, PRIVATE_KEY_SIZE))
    };
  }
};
...
var Secp256r1Keypair = class _Secp256r1Keypair extends Keypair {
  ...
  static fromSecretKey(secretKey, options) {
    ...
    if (!options || !options.skipValidation) {
      const encoder = new TextEncoder();
      const signData = encoder.encode("sui validation");
      const msgHash = (0, import_utils6.bytesToHex)((0, import_blake2b6.blake2b)(signData, { dkLen: 32 }));
  ...
  export() {
    return {
      schema: "Secp256r1",
      privateKey: (0, import_bcs23.toB64)(this.keypair.secretKey)
    };
  }
};
...
function toParsedSignaturePubkeyPair(serializedSignature) {
  const bytes = (0, import_bcs24.fromB64)(serializedSignature);
  const signatureScheme = SIGNATURE_FLAG_TO_SCHEME[bytes[0]];
  if (signatureScheme === "MultiSig") {
    try {
...
function fromExportedKeypair(keypair) {
  const secretKey = (0, import_bcs24.fromB64)(keypair.privateKey);
...
var import_bcs30 = require("@mysten/bcs");
...
var TransferObjectsTransaction = (0, import_superstruct15.object)({
  ...
  address: PureTransactionArgument(import_bcs25.BCS.ADDRESS)
});
    ...
    return create(
      {
        kind: "Publish",
        modules: modules.map(
          (module2) => typeof module2 === "string" ? Array.from((0, import_bcs25.fromB64)(module2)) : module2
        ),
...
function hashTypedData(typeTag, data) {
  ...
  return (0, import_blake2b7.blake2b)(dataWithTag, { dkLen: 32 });
}
...
var TransactionBlockDataBuilder = class _TransactionBlockDataBuilder {
  ...
  static getDigestFromBytes(bytes) {
    ...
    return (0, import_bcs27.toB58)(hash);
  }
  build({
...
var _TransactionBlock = class _TransactionBlock {
  ...
  static fromKind(serialized) {
    ...
    __privateSet(tx, _blockData, TransactionBlockDataBuilder.fromKindBytes(
      typeof serialized === "string" ? (0, import_bcs29.fromB64)(serialized) : serialized
    ));
    return tx;
  }
  /**
...
var JsonRpcProvider = class {
  ...
  async executeTransactionBlock(input) {
    return await this.client.requestWithType(
      ...
      [
        typeof input.transactionBlock === "string" ? input.transactionBlock : (0, import_bcs30.toB64)(input.transactionBlock),
  ...
  async devInspectTransactionBlock(input) {
    ...
    if (isTransactionBlock(input.transactionBlock)) {
      ...
      devInspectTxBytes = (0, import_bcs30.toB64)(
      ...
      devInspectTxBytes = (0, import_bcs30.toB64)(input.transactionBlock);
    } else {
  ...
  async getChainIdentifier() {
    ...
    const bytes = (0, import_bcs30.fromB58)(checkpoint.digest);
    return (0, import_bcs30.toHEX)(bytes.slice(0, 4));
  }
...
var SignerWithProvider = class {
  ...
  async signMessage(input) {
    ...
    return {
      messageBytes: (0, import_bcs31.toB64)(input.message),
      signature
    };
  }
  ...
  async signTransactionBlock(input) {
    ...
    return {
      transactionBlockBytes: (0, import_bcs31.toB64)(transactionBlockBytes),
      signature
    };
  }
  /**
  ...
  async dryRunTransactionBlock(input) {
    ...
    if (isTransactionBlock(input.transactionBlock)) {
      ...
      dryRunTxBytes = (0, import_bcs31.fromB64)(input.transactionBlock);
    } else if (input.transactionBlock instanceof Uint8Array) {
...
var RawSigner = class _RawSigner extends SignerWithProvider {
  ...
  async signData(data) {
    ...
    const digest = (0, import_blake2b8.blake2b)(data, { dkLen: 32 });
...
var import_blake2b9 = require("@noble/hashes/blake2b");
async function verifyMessage(message, serializedSignature, scope) {
  ...
  if (scope === 3 /* PersonalMessage */) {
    const messageBytes2 = messageWithIntent(
      scope,
      bcs.ser(["vector", "u8"], typeof message === "string" ? (0, import_bcs33.fromB64)(message) : message).toBytes()
    );
    if (await signature.pubKey.verify((0, import_blake2b9.blake2b)(messageBytes2, { dkLen: 32 }), signature.signature)) {
      return true;
    }
    const unwrappedMessageBytes = messageWithIntent(
      scope,
      typeof message === "string" ? (0, import_bcs33.fromB64)(message) : message
    );
    return signature.pubKey.verify(
      (0, import_blake2b9.blake2b)(unwrappedMessageBytes, { dkLen: 32 }),
      signature.signature
    );
  }
  ...
  return signature.pubKey.verify((0, import_blake2b9.blake2b)(messageBytes, { dkLen: 32 }), signature.signature);
}

// src/index.ts
var import_bcs35 = require("@mysten/bcs");
